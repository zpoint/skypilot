...
def test_1_direct_kubectl_no_pipe():
    ...
    cmd = [
        "kubectl", "exec", "-i", "--pod-running-timeout", "30s", "-n",
        NAMESPACE, "--context", CONTEXT, f"pod/{POD_NAME}", "--", "/bin/bash",
        "-c", "for i in {1..200000}; do echo $i; done"
    ]
...
def test_4_kubectl_with_shell_tee():
    ...
    print("\n" + "=" * 80)
    print(
        "TEST 4: kubectl with shell | tee /dev/null (mimics process_stream=False)"
    )
    print("=" * 80)

    # This mimics what happens when process_stream=False
    cmd = (f"kubectl exec -i --pod-running-timeout 30s "
...
def main():
    ...
    tests = [
        ...
        ("Test 2: PIPE but no read (should fail)",
         test_2_kubectl_with_pipe_no_read),
        ("Test 3: PIPE with active reading", test_3_kubectl_with_pipe_and_read),
        ("Test 4: Shell tee (process_stream=False)",
         test_4_kubectl_with_shell_tee),
        ("Test 5: PIPE with file writing",
         test_5_kubectl_with_context_simulation),
    ]
